  ...
  Future<List<Map<String, dynamic>>> _fetchNewOrders() async {
    ...
    final res = await sb
        ...
        .select('''
          ...
          awarded_order_version:order_versions!orders_awarded_order_version_id_fkey(
            id,
    ...
    bool hasActiveAssignment(Map<String, dynamic> row) {
      final awardedOrderVersion = row['awarded_order_version'];
      if (awardedOrderVersion is! Map) return false;

      final assignments = awardedOrderVersion['assignments'];
                      ...
                      if (assignments is List) {
                        for (final a in assignments) {
                          if (a is Map && a['is_active'] == true) {
                            final bt = a['budget_typology'];
                            final pt = a['product_type'];

                            typologyName =
                                (bt is Map ? bt['name'] : null)?.toString() ?? '';
                            productTypeName =
                                (pt is Map ? pt['name'] : null)?.toString() ?? '';
                          ...
                          children: [
                            ...
                            SizedBox(
                              ...
                              child: IconButton(
                                ...
                                tooltip: 'Editar',
                                onPressed: () async {
                                  // valores atuais (do row + assignment ativo)
                                  int? currentTypologyId;
                                  ...
                                  if (assignments is List) {
                                    for (final a in assignments) {
                                      if (a is Map && a['is_active'] == true) {
                                        currentTypologyId = (a['budget_typology_id'] as num?)?.toInt();
                                        currentProductTypeId = (a['product_type_id'] as num?)?.toInt();
                                        break;
                                      }
                                    }
                                  }

                                  final currentPhaseId = (row['budgeting_phase_id'] as num?)?.toInt();
                                  ...
                                  final changed = await showDialog<bool>(
                                    ...
                                    builder: (_) => EditBudgetDialog(
                                      ...
                                      orderRef: orderRef,
                                      customerName: customerName,
                                      currentBudgetingPhaseId: currentPhaseId,
                                      currentBudgetingPhaseName: phaseName,
                                      currentTypologyId: currentTypologyId,
                                      currentTypologyName: typologyName,
                                      currentProductTypeId: currentProductTypeId,
                                      currentProductTypeName: productTypeName,
                                      currentIsSpecial: isSpecial,
                                    ),
                                  );

                                  debugPrint('Edit dialog result: $changed');
                                },
                              ),
                            ),
                          ],
